...
int jump_pos[17] = {0,4,8,12,16,20,24,28,30,28,24,20,16,12,8,4,0};
int dino_moved=1;
int jump_counter = 0;
int jumping = 0;
...
int draw_obstacle(void) {
    rectangle(obs.x[0], obs.y[0], obs.x[0] + obs.width, obs.y[0] + obs.height, GREEN);
	return 0;
}

int clear_obstacle(void) {
    rectangle(obs.x[0], obs.y[0], obs.x[0] + obs.width, obs.y[0] + obs.height, BLACK);
	return 0;
}
