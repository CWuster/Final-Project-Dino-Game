#include "./Generated_Code/modified_cactus.h"
...
int main(int argc, char *argv[]) {
  ...
  int m =
      ((int)sizeof(modified_cactus_rom) / (int)sizeof(modified_dino_rom[0]));
  int n = ((int)sizeof(modified_cactus_rom[0]) /
           (int)sizeof(modified_cactus_rom[0][0]));
  printf("%d\n", m);
  printf("%d\n", n);
  for (int j = 0; j < m; ++j) {
    printf("{");
